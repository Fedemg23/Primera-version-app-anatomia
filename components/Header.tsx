...
const StatItem = memo(React.forwardRef<HTMLButtonElement, { 
    ...
    >
        ...
        <span className={`font-black text-[#3A3A3A] ml-1 sm:ml-1.5 tabular-nums min-w-[2ch] text-center ${textClass || 'text-lg sm:text-xl md:text-2xl'}`}>{value}</span>
    </button>
)));
        ...
        <div className="w-full z-10">
            <div className="w-full h-24 md:h-28 bg-transparent">
                <div className="max-w-7xl mx-auto h-full grid grid-cols-3 items-start gap-1 sm:gap-4 md:gap-8 py-2 px-3">
                    {/* Left Side: Back Button or Profile and Experience */}
                    <div className="flex-shrink-0 flex justify-start items-center gap-2 sm:gap-3 md:gap-4 ml-2">
                        {showBackButton ? (
                            ...
                            <div className="flex flex-col gap-2">
                                ...
                                {/* Gift button for level rewards */}
                                {pendingLevelRewards && (
                                    <button 
                                        onClick={() => onNavigate('level_rewards')} 
                                        title="Ver Recompensas de Nivel" 
                                        className="relative self-start p-2 rounded-full bg-amber-500/20 border border-amber-500/30 active:bg-amber-500/30 transition-colors active:scale-95 touch-manipulation"
                                    >
                                        <Gift className="w-5 h-5 text-amber-400" />
                                        <span className="absolute top-0 right-0 block h-2.5 w-2.5 rounded-full bg-red-500 ring-2 ring-slate-950 animate-notification-pulse"></span>
                                    </button>
                                )}
                            </div>
                        )}
